...
public class RemapClass implements Serializable {
    ...
    public final RemapperFormat format;
    ...
    private void writeObject(ObjectOutputStream oos) throws IOException {
        if (RemapClassSerializeHelper.getInstance().reverse) {
            ...
            String[] sups = superClasses.stream().filter(Objects::nonNull).map(clazz -> clazz.remapName)
                    .toArray(String[]::new);
            ...
            String[] sups = superClasses.stream().filter(Objects::nonNull).map(clazz -> clazz.sourceName)
                    .toArray(String[]::new);
    ...
    private Map<String, String> reverseStringMap(Map<String, String> map) {
        ...
        map.forEach((s1, s2) -> rev.put(s2, s1));
        return rev;
    }
