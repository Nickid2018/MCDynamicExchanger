...
public class MethodAnalyzer extends MethodVisitor implements Opcodes {
    ...
    public void visitParameter(String name, int access) {
        asmifier.line("mv.visitParameter(%s, %s);",
                asmifier.quote(name), AccessFlagCategory.fromIntToAccessFlag(access, AccessFlagCategory.PARAMETER, asmifier));
    }
    ...
    public void visitFrame(int type, int numLocal, Object[] local, int numStack, Object[] stack) {
        if (asmifier.noFrames)
            return;
        StringBuilder localStr = new StringBuilder("new Object[]{ ");
        if (local != null && local.length != 0) {
            localStr.append(MethodOpcodes.getAsFrameObject(local[0], this));
            for (int i = 1; i < local.length; i++)
                localStr.append(", ").append(MethodOpcodes.getAsFrameObject(local[i], this));
        }
        localStr.append(" }");
        StringBuilder stackStr = new StringBuilder("new Object[]{ ");
        if (stack != null && stack.length != 0) {
            stackStr.append(MethodOpcodes.getAsFrameObject(stack[0], this));
            for (int i = 1; i < stack.length; i++)
                stackStr.append(", ").append(MethodOpcodes.getAsFrameObject(stack[i], this));
        }
        stackStr.append(" }");
        asmifier.line("mv.visitFrame(%s, %d, %s, %d, %s);",
                MethodOpcodes.getAsFrameType(type), numLocal, localStr, numStack, stackStr);
    }

    @Override
    public void visitInsn(int opcode) {
        if(asmifier.noConvertConstants)
            asmifier.line("mv.visitInsn(%d);", opcode);
        else
    ...
    public void visitIntInsn(int opcode, int operand) {
        if(asmifier.noConvertConstants)
            asmifier.line("mv.visitIntInsn(%d, %d);", opcode, operand);
    ...
    public void visitVarInsn(int opcode, int var) {
        if(asmifier.noConvertConstants)
            asmifier.line("mv.visitVarInsn(%d, %d);", opcode, var);
        else
    ...
    public void visitTypeInsn(int opcode, String type) {
        if(asmifier.noConvertConstants)
            asmifier.line("mv.visitTypeInsn(%d, %s);", opcode, asmifier.quote(type));
        else
    ...
    public void visitFieldInsn(int opcode, String owner, String name, String descriptor) {
        if(asmifier.noConvertConstants)
            asmifier.line("mv.visitFieldInsn(%d, %s, %s, %s);", opcode,
    ...
    public void visitMethodInsn(int opcode, String owner, String name, String descriptor, boolean isInterface) {
        if(asmifier.noConvertConstants)
            asmifier.line("mv.visitMethodInsn(%d, %s, %s, %s, %s);", opcode,
    ...
    private String parseHandleAsString(Handle handle) {
        return String.format("new Handle(%s, %s, %s, %s, %s)",
                MethodOpcodes.getAsHandleType(handle.getTag(), asmifier), asmifier.quote(handle.getOwner()),
    ...
    public int checkLabel(Label label) {
        int index = labels.indexOf(label);
        if (index < 0) {
            asmifier.line("Label l%d = new Label();", index = labels.size());
            labels.add(label);
        }
        return index;
    }
    ...
    public void visitJumpInsn(int opcode, Label label) {
        checkLabel(label);
        if(asmifier.noConvertConstants)
            asmifier.line("mv.visitJumpInsn(%d, l%d);", opcode, labels.indexOf(label));
        else
    ...
    public AnnotationVisitor visitTryCatchAnnotation(int typeRef, TypePath typePath, String descriptor, boolean visible) {
        String typeRefStr;
        if (!asmifier.noConvertConstants && typeRef == TypeReference.EXCEPTION_PARAMETER)
